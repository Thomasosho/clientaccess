...
export class AddQualificationComponent implements OnInit {
  ...
  addQualification() {
    ...
    this.qualificationForm.value.artisanId = Number(this.utilService.getUser());
    this.crudService.postRequest('/api/artisanqualification/add', this.qualificationForm.value).subscribe((res: any) => {
      console.log(res);
      if (res.code === 0) {
