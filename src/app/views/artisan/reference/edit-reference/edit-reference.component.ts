...
export class EditReferenceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.refereeForm = this.form.group({
      ...
      phoneNumber: [this.document.phoneNumber, Validators.required],
    });
  }
  ...
  editReferee() {
    ...
    this.refereeForm.value.artisanId = this.document.artisanId;
    this.crudService.putRequest('api/artisanreferences/edit', this.refereeForm.value).subscribe((res: any) => {
      console.log(res);
      if (res.code === 0) {
        this.swalService.success(res.message);
        this.router.navigate(['/artisan/referee']);
      } else {
        this.swalService.error('An Error Occured');
      }
      this.loading = false;
