...
export class RegisterSupplierComponent implements OnInit {
  ...
  register() {
    ...
    this.loading = true;
    this.crudService.postRequestNoAuth('api/supplier/add', this.registerForm.value).subscribe((res: any) => {
      console.log(res);
      this.loading = false;
      if (res.code === 0) {
        this.swalService.success('Supplier registered successfully');
      ...
      this.swalService.error('There was an error registering the supplier, please try again.');
    });
  }
