...
export class RegisterExpertComponent implements OnInit {
  ...
  register() {
    ...
    this.loading = true;
    this.crudService.postRequestNoAuth('api/expert/add', this.registerForm.value).subscribe((res: any) => {
      console.log(res);
      this.loading = false;
      if (res.code === 0) {
        this.swalService.success('Expert registered successfully');
      ...
      this.swalService.error('There was an error registering the expert, please try again.');
    });
  }
